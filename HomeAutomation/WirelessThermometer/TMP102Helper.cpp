...
******************************************************************************/
#include "TMP102Helper.h"
#include <Wire.h>

namespace HomeAutomationTools {
namespace {
     const unsigned TEMPERATURE_REGISTER = 0x00;
     const unsigned CONFIG_REGISTER = 0x01;
     const unsigned T_LOW_REGISTER = 0x02;
     const unsigned T_HIGH_REGISTER = 0x03;
}
    ...
    void TMP102::setOneShotMode(void)
    {
    ...
    void TMP102::setContinuousMode(void)
    {
        byte registerByte; // Store the data from the register here
    ...
    float TMP102::readTempCfromShutdown()
    {	// do one-shot temperature read
        openPointerRegister(CONFIG_REGISTER);
        byte registerByte = readRegister(0);

        registerByte |= 0x81u;	// set OS (one-shot) bit
        ...
        static const unsigned MAX_WAIT = 100;
        for (unsigned f = 0; f < MAX_WAIT; f++)
        {
            delay(1);
            openPointerRegister(CONFIG_REGISTER);
            registerByte = readRegister(0);
            if (registerByte & 0x80u)
                return readTempC();
        }
        return -400.f;
    }
